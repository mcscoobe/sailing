...
public interface SailingConfig extends Config
{
	...
	@ConfigItem(
		keyName = "reorderFacilityRows",
		name = "Unlock Facility Row Reordering",
		description = "Enable manual drag-and-drop reordering of facility rows. When enabled, right-click facility rows to move them around. Turn off to lock the current order in place.",
		section = SECTION_FACILITIES,
		position = 8
	)
	default boolean reorderFacilityRows()
	{
		return false;
	}

	@ConfigItem(
		keyName = "facilityDragOutlineColor",
		name = "Draggable Outline Colour",
		description = "Colour for outlining draggable facility rows when reorder mode is enabled.",
		section = SECTION_FACILITIES,
		position = 9
	)
	@Alpha
	default Color facilityDragOutlineColor()
	{
		return Color.CYAN;
	}

	@ConfigItem(
		keyName = "facilityDragActiveColor",
		name = "Active Drag Colour",
		description = "Colour for highlighting the currently dragged facility row.",
		section = SECTION_FACILITIES,
		position = 10
	)
	@Alpha
	default Color facilityDragActiveColor()
	{
		return Color.ORANGE;
	}

	@ConfigItem(
		keyName = "facilityDropTargetColor",
		name = "Drop Target Colour",
		description = "Colour for highlighting valid drop targets during drag operations.",
		section = SECTION_FACILITIES,
		position = 11
	)
	@Alpha
	default Color facilityDropTargetColor()
	{
		return Color.GREEN;
	}

	@ConfigItem(
		keyName = "facilityInvalidDropColor",
		name = "Invalid Drop Area Colour",
		description = "Colour for highlighting invalid drop areas during drag operations.",
		section = SECTION_FACILITIES,
		position = 12
	)
	@Alpha
	default Color facilityInvalidDropColor()
	{
		return Color.RED;
	}
